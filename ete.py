...
def extract(img_path, use_tf, no_dewarp, out_path) -> str:
    #removes datatype
    f_name = os.path.splitext(img_path)[0]
    print(f_name)

    pkl_path = pathlib.Path(os.path.splitext(f_name)[0] + f"{f_name}.pkl")
    if pkl_path.exists():
        # Load from cache
        ...
        staff, symbols, stems_rests, notehead, clefs_keys = generate_pred(str(img_path), use_tf)#use_tf=use_tf)
...
def main(img_path, use_tf, dewarp, out_path) -> None:
    ...
    clear_data()
    mxl_path = extract(img_path, use_tf, dewarp, out_path)
    img = teaser()
    img.save(mxl_path.replace(".musicxml", "_teaser.png"))
    
#make sure to use doublebackslashes otherwise it WILL NOT work
input_path = "" #add here your path to the image
output_path = "" #add here your path to the folder where the results should be saved

#path(str); use tf(bool); no dewarp?(bool); out_path(str)
main(input_path, True, True, output_path)